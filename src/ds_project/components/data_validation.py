...
class DataValidation:
    ...
    def preprocess_data(self):
        try:
            ...
            final_rating_50 = final_rating[final_rating['num_of_rating'] >= 50]

            # lets drop the duplicates
            final_rating_50.drop_duplicates(['user_id','title'],inplace=True)
            logging.info(f" Shape of the final clean dataset: {final_rating_50.shape}")
                        
            # Saving the cleaned data for transformation
            os.makedirs(self.data_validation_config.clean_data_dir, exist_ok=True)
            final_rating_50.to_csv(os.path.join(self.data_validation_config.clean_data_dir,'clean_data.csv'), index = False)
            ...
            pickle.dump(final_rating_50,open(os.path.join(self.data_validation_config.serialized_objects_dir, "final_rating.pkl"),'wb'))
